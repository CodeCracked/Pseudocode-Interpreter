...
public class ConstantStatementRule implements IGrammarRule
{
    @Override
    public Result<AbstractNode> build(Parser parser)
    {
        ...
        if (!keyword.isKeyword(TokenType.STATEMENT_KEYWORD, "Constant", 1)) return result.failure(new SyntaxException(parser, "Expected keyword 'Declare'! Did you forget proper capitalization?"));
